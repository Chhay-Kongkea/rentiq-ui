...
export default function VendorEarningsPage() {
  ...
  const { data: report, isLoading, isError } = useGetVendorEarningsQuery({ from, to, groupBy, page: 0, size: 100 });
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Selected period</p><p className="mt-2 text-2xl font-extrabold text-slate-950">{money(report?.totalEarnings)}</p><p className="mt-1 text-xs text-slate-500">{report?.totalTransactions ?? 0} transactions</p></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">All-time earnings</p><p className="mt-2 text-2xl font-extrabold text-slate-950">{money(performance?.totalEarnings)}</p><p className="mt-1 text-xs text-slate-500">From vendor performance</p></div>
        ...
        <div className="divide-y divide-slate-100">
          {rows.map((row, index) => (
            <div key={`${row.period}-${index}`} className="grid grid-cols-[1fr_auto_auto] items-center gap-6 px-5 py-4">
              <div><p className="text-sm font-semibold text-slate-900">{row.period || "Period"}</p><p className="text-xs text-slate-500">{row.transactionCount ?? 0} transactions</p></div>
              <div className="hidden h-2 w-40 overflow-hidden rounded-full bg-slate-100 sm:block"><div className="h-full rounded-full bg-[#253C95]" style={{ width: `${report?.totalEarnings ? Math.min(100, ((row.totalEarnings ?? 0) / report.totalEarnings) * 100) : 0}%` }} /></div>
              <p className="text-sm font-extrabold text-slate-950">{money(row.totalEarnings)}</p>
            </div>
          ))}
